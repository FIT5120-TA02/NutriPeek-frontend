...
export default function GameCard({ card, onFlip, isDisabled }: GameCardProps) {
  // Handle card click
  const handleClick = () => {
    ...
    >
      ...
      >
        ...
        <div className={`absolute w-full h-full backface-hidden rounded-xl shadow-md border-2 border-white flex items-center justify-center 
          ...
          <span className="text-4xl text-white font-bold">{card.isMatched ? '✓' : '?'}</span>
        </div>
        
        {/* Card front (food image) */}
        <div className={`absolute w-full h-full backface-hidden rotate-y-180 rounded-xl shadow-md border-2 p-2 flex flex-col items-center justify-center
